...
import useFetchBlogs, {
  BlogValueType,
} from "admin/pages/FileMaintenance/pages/Webinars/hooks/useFetchBlogs";
...
const BlogForm: React.FC = () => {
  ...
  const editInitialValues: Omit<BlogValueType, "role"> = {
    ...
    thumbnailAlt: blogs[0]?.thumbnailAlt,
    title: blogs[0]?.title,
  ...
  const submitBlogFormHandler = (values: any) => {
    ...
    isEditMode
      ? axios
          .put(
            isEditMode
              ? ENDPOINTS.BLOGS_SINGLE.replace(":blogId", blogs[0]._id ?? "")
              : ENDPOINTS.BLOGS,
            {
              metaTagTitle: values.metaTagTitle.toString(),
              metaTagDescription: values.metaTagDescription.toString(),
              thumbnailAlt: values.metaTagDescription.toString(),
                  ...
                  >
                    ...
                    <ComponentValidator showNull={!values.thumbnail}>
                      <div className="alt-input">
                        <FormikTextInput
                          name="thumbnailAlt"
                          label="Thumbnail Alt Attribute"
                          value={values.thumbnailAlt}
                          variant="filled"
                          InputLabelProps={{ shrink: !!values.thumbnailAlt }}
                        />
                      </div>
                    </ComponentValidator>
                  </Grid>
