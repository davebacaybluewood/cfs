...
import { ILabeledInput } from "./Form";
      ...
      >
        <div className="event-card-content">
          <div className="date-wrapper">
            <CalendarTodayIcon />
            {formatISODateToDate(props.event_date.toString())} PST
          </div>
          <h1>{props.title}</h1>
          ...
          <Button
            onClick={() =>
              navigate(paths.events.replace(":id", "639ce4ff061e6ed3a75acf51"))
            }
          >
            VIEW MEDIA
          </Button>
      ...
      <Formik
        ...
        onSubmit={(data: EventValues, { setSubmitting, resetForm }) => {
          ...
          props.setClipboardValue(
            `${MAIN_LOCALHOST}${paths.events.replace(":id", referenceId)}`
          );
